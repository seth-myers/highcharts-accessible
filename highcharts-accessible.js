...
(function(H) {
  ...
  H.wrap(H.Chart.prototype, 'init', function(proceed, options) {
    ...
    var accOptions = {
      ...
      autoAccessible: (options.accessibleSettings.autoAccessible === undefined) ? true : options.accessibleSettings.autoAccessible, // default to auto-accessible feature on
      chartType: findChartTypes(options),
      csvLinkText: options.accessibleSettings.csvLinkText || "Download chart data.",
      langAttribute: options.accessibleSettings.langAttribute || "en", // default to english,
      timeSeries: (options.accessibleSettings.timeSeries === undefined) ? true : options.accessibleSettings.timeSeries, // default to time series
      userDesc: options.accessibleSettings.desc || null,
      userTitle: options.accessibleSettings.title || null,
      xUnitPrefix: options.accessibleSettings.xUnitPrefix || "",
      xUnitSuffix: options.accessibleSettings.xUnitSuffix || "",
      yUnitPrefix: options.accessibleSettings.yUnitPrefix || "",
      yUnitSuffix: options.accessibleSettings.yUnitSuffix || ""
    };
  ...
  function processDesc(o, d) {
    ...
    for (var i = 0; i < series.length; i++) {
      ...
      if (seriesType == 'pie') {
        var total = yVals.reduce(function(a, b) {
          return a + b;
        });
      ...
      if (o.timeSeries) {
        ...
        if (combinedSeriesLen < 15) {
          if (seriesType == 'line' || seriesType == 'bar' || seriesType == 'column') {
            for (var iii = 0; ii < xValsLen; iii++) {
              theReturn += o.xUnitPrefix + yVals[iii] + o.xUnitSuffix + ", " + xLabels[iii] + ", " + sName + ". ";
            }
          }
  ...
  function setTags(tTag, dTag, chartData, userOptions) {
    ...
    descNode.firstChild.nodeValue = dTag;
